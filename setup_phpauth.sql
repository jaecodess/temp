...
-- 1. Extend members with a nullable INT link column to phpauth_users
ALTER TABLE members ADD COLUMN phpauth_uid INT NULL AFTER role;

-- 2. PHPAuth core tables (from vendor)
-- MySQL dump
...
-- 3. Add username column to phpauth_users (set via register() params)
ALTER TABLE phpauth_users ADD COLUMN username VARCHAR(20) NULL AFTER email;

-- 4. Migrate existing members into phpauth_users with isactive=1
--    (PHPAuth's register() uses $use_email_activation=false by default = auto-activate,
--    but for migration we INSERT directly with isactive=1)
INSERT INTO phpauth_users (email, password, isactive, username)
SELECT email, password, 1, username FROM members;

-- 5. Back-fill members.phpauth_uid using phpauth_users.id (INT)
UPDATE members m
JOIN phpauth_users pu ON m.email = pu.email
SET m.phpauth_uid = pu.id;

-- 6. Invalidate members.password for all migrated accounts.
-- PHPAuth now owns the authoritative password in phpauth_users.
-- '!' is an invalid bcrypt hash that can never match via password_verify().
